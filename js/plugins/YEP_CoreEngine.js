...
//=============================================================================
/*:
 * @plugindesc v1.10 Necesario para la mayor parte de Engine Yanfly.
 * También contiene correcciones de bugs del RPG Maker.
 ...
 *
 * @param Scale Title
 * @desc Deseas que el fondo del título se ajuste a la pantalla?
 * NO - false     SI - true
 * @default true
 *
 * @param Scale Game Over
 * @desc Quieres escalar la pantalla de Juego Terminado a la 
 * resolución?  NO - false     SI - true
 * @default true
 *
 * @param Open Console
 * @desc Para propósitos de prueba y depuración, esto abre la consola.
 * No abrir - false     Abrir - true
 * @default false
 *
 * @param Reposition Battlers
 * @desc Permitir que el plugin reubique los combatientes según la
 * resolución de la pantalla?     NO - false  SI - true
 * @default true
 *
 * @param ---Gold (Oro)---
 * @desc
 *
 * @param Gold Max
 * @desc La máxima cantidad de oro que el jugador puede tener.
 * Por defecto: 99999999
 * @default 99999999
 *
 * @param Gold Font Size
 * @desc El tamaño de la fuente utilizado para mostrar el oro.
 * Por defecto: 28
 * @default 20
 *
 * @param Gold Icon
 * @desc Este será el icono utilizado para representar el oro en
 * la ventana. Si se deja en 0, Ningún icono será mostrado.
 * @default 313
 *
 * @param Gold Overlap
 * @desc Esto se mostrara si el número de oro excede el tamaño del
 * área del contenido dado.
 * @default A lotta
 *
 * @param ---Items (Objetos)---
 * @desc
 *
 * @param Default Max
 * @desc Este es el máximo numero de items que el personaje puede
 * tener.   Por defecto: 99
 * @default 99
 *
 * @param Quantity Text Size
 * @desc Este es el tamaño de la fuente del texto usado para la 
 * cantidad de items.     Por defecto: 28
 * @default 20
 *
 ...
 *
 * @param Max Level
 * @desc Ajusta el limite máximo de nivel para los personajes.
 * Por defecto: 99
 * @default 99
 *
 * @param Actor MaxHP
 * @desc Ajusta el limite máximo de HP para los personajes.
 * Por defecto: 9999
 * @default 9999
 *
 * @param Actor MaxMP
 * @desc Ajusta el limite máximo de MP para los personajes.
 * Por defecto: 9999
 * @default 9999
 *
 * @param Actor Parameter
 * @desc Ajusta el limite máximo de parámetros para los personajes.
 * Por defecto: 999
 * @default 999
 *
 * @param Enemy MaxHP
 * @desc Ajusta el limite máximo de HP para los enemigos.
 * Por defecto: 999999
 * @default 999999
 *
 * @param Enemy MaxMP
 * @desc Ajusta el limite máximo de MP para los enemigos.
 * Por defecto: 9999
 * @default 9999
 *
 * @param Enemy Parameter
 * @desc Ajusta el limite máximo de parámetros para los enemigos.
 * Por defecto: 999
 * @default 999
 *
 ...
 * @desc Ajusta el ritmo de las animaciones en batalla. Menos para
 * más rápido.   Por defecto: 4
 * @default 4
 *
 * @param Flash Target
 ...
 *
 * @param Default Font
 * @desc Fuente(s) por defecto para todos los demás.
 * Por defecto: GameFont
 * @default GameFont, Verdana, Arial, Courier New
 *
 * @param Font Size
 * @desc Tamaño de letra predeterminado utilizado para las 
 * ventanas.    Por defecto: 28
 * @default 28
 *
 * @param Text Align
 ...
 * @desc Agrupa dígitos con una coma.
 * false - APAGADO     true - ENCENDIDO
 * @default true
 *
 * @param Line Height
 * @desc Ajuste de la altura de la línea universal utilizada en 
 * Windows.   Por defecto: 36
 * @default 36
 *
 * @param Icon Width
 ...
 * @desc Ajusta el relleno a todas las ventanas estándar. 
 * Por defecto: 18
 * @default 18
 *
 * @param Text Padding
 ...
 *
 * @param Menu TP Bar
 * @desc Dibuja una barra de TP en el menú de estado para los 
 * personajes.  false - APAGADO     true - ENCENDIDO
 * @default true
 *
 ...
 * Este plugin tiene como función, principalmente, corregir errores y permitir 
 * al usuario mayor control sobre las varias características de RPG MAKER MV,
 * como la resolución de la pantalla, fuentes, colores de ventanas, y más.
 *
 * Solo hay que poner en la parte superior de todos los demás Yanfly Engine 
 * Plugins. Ajuste cualquier parámetro como mejor le parezca.
 *
 * ============================================================================
 ...
 *
 * Animation Overlay (Superposicion de Animación)
 * Cuando una habilidad/objeto que apunta a múltiples enemigos de una vez
 * se utiliza con la animación en pantalla completa, que cubre múltiples 
 * veces causando que la imagen parezca distorsionada por una serie de 
 * efecto cubridor. Este plugin soluciona este problema por tener 
 * apenas una animación siendo ejecutada sobre un grupo en vez de cada una.
 *
 * Event Updating (Actualización de Evento)
 ...
 *   La velocidad de movimiento de eventos es levemente más lenta de lo que 
 *   debería debido a un pequeño error en el código fuente. El plugin soluciona 
 *   este problema y permite que se mueva a la velocidad correcta.
 *
 * Event Movement Queue (Movimiento de eventos en Cola)
 *   Si un evento se moviera a través de un evento de comando, cambia una 
 *   condición que establece el evento a mover para una página diferente, 
 *   causaría que la ruta de movimiento del evento se detenga. El plugin 
 *   soluciona este problema y la ruta del evento finalizara.
 *
 * Event Colliding (colisión de Evento)
 *   Los eventos no se pueden mover por encima de otros eventos con una 
 *   configuración por debajo del jugador. Esto hace que sea difícil para 
 *   ciertos tipos de eventos existir. el plugin soluciona este problema al 
 *   hacer la prueba de colisión solo aplica para eventos con prioridad  
 *   "Same as Characters". Cualquier evento que este encima o abajo de los 
 *   personajes no entrara en colisión con otros eventos.
 *
 * Screen Tearing(Lagrimeo de pantalla)
 *   Cuando se mueve lentamente, los tiles en la pantalla lagrimean. Si bien no 
 *   es perceptible en todos los sistemas, equipos más lentos definitivamente 
 *   los mostraran. El plugin soluciona este problema y sincroniza los tiles 
 *   para permanecer correctamente en ritmo con el movimiento de la cámara 
 *   en la pantalla.
 *
 * Sprite Distortion (Distorsion de Sprite )
 *   Debido a un extraño comportamiento matemático de JavaScript, a veces los 
 ...
 * límite del editor, que es 9,999,999 limit. También puede agregar tags de 
 * notas en items, armas, y armaduras por encima del límite de coste 999,999
 * 
 *
 * Comandos de plugin:
 ...
 *   límite de costo de oro del editor, que es 999,999.
 *
 * Enemy Notetag(Etiqueta Enemigo)
 *   <Gold: x>
 *  Cambia el valor de la caída de oro al enemigo x. Esta etiqueta le permite 
 *  que usted ignore el límite de caída de oro del editor, que es 9,999,999.
 *
 * ============================================================================
 * Items (Objetos)
 * ============================================================================
 *
 * Cambia los parámetros para reflejar el número máximo de items de un 
 * jugador que puede tener por item. Si quieres hacer que los items 
 * individuales tengan diferentes valores máximos,
 * usa la siguiente etiqueta:
 *
 * Notetag de Item, Arma, y Armadura::
 *   <Max Item: x>
 *   Esto cambia la cantidad máxima del item a x.
 *
 * ============================================================================
 * Stats (Estadisticas)
 * ============================================================================
 *
 * Incluso con los límites de los parámetros aumentados, el editor todavía 
 * esta confinado para el límite de patrones por defecto de RPG Maker MV. 
 * Para pasar por ellos, use las siguientes etiquetas para permitir
 * más control sobre los aspectos individuales para los parámetros.
 *
 * Actor Notetag(Etiqueta de actor)
 *   <Initial Level: x>
 *  Cambia el nivel inicial del actor a x. Esto le permite ignorar el
 *   límite de nivel del editor, que es 99.
 *
 *   <Max Level: x>
 *   Cambia el nivel máximo del actor a x. Esto le permite ignorar el nivel
 *   límite del editor, que es 99.
 *
 ...
 *  equipamiento ir más allá de la limitación por defecto del editor siempre 
 *  y cuando el valor máximo lo permita.
 *
 ...
 *   el estado especifico. Esto permite a la pieza de equipamiento ir más allá 
 *   de la limitacion por defecto del editor.
 *
 *   <exp: x>
 *   Esto cambia la exp dada por el enemigo por x cantidad. Esto permite que el
 *   enemigo de más exp que el limite por defecto del editor, que es 9,999,999.
 *
 * ============================================================================
 * Changelog (Historial de cambios)
 * ============================================================================
 *
 * Version 1.10: (Implementado)
 * - Removed an MV bugfix that was applied through MV's newes tupdate.
 *
 * Version 1.09: (Implementado)
