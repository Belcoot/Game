...
function MakerManager() {
	throw new Error('You silly-billy, MakerManager is a static class! (ï½¡â€¢Ì€á´—-)âœ§');
}

function FileManager() {
	throw new Error('Hey, noob! FileManager is a static class! ãƒ¾(Ð¾-Ï‰ï½¥)ï¾‰âŒ’â˜…');
}

function DataManagerEX() {
	throw new Error('It doesn\'t matter how extreme, DataManagerEX is a static class! (â•¬Ã’^Ã“)');
}

function DebugManager() {
	throw new Error('Uh... DebugManager is a static class! (/Îµï¼¼*)');
}

function WindowManager() {
	throw new Error('Great job. WindowManager is a static class! Ê•Â·á´¥Â·ã€€Ê”');
}
